...
public class Lista<T> implements Iterable<T> {
    ...
    public void agregaFinal(T elemento) {
        if(elemento == null) throw new IllegalArgumentException("no se puede");
    ...
    public void agregaInicio(T elemento) {
        if(elemento == null) throw new IllegalArgumentException("no se pudo");
        Nodo nodo = new Nodo(elemento);
        if (esVacia()){
            cabeza = nodo;
            rabo = cabeza;
    ...
    public void inserta(int i, T elemento) {
        if (elemento == null) {
            throw new IllegalArgumentException("No hay elemento que insertar");
        } else if (i <= 0) {
    ...
    public int indiceDe(T elemento) {
        ...
        int indice = 0;
        while(nodo != null){
            if(nodo.elemento.equals(elemento)) return indice;
            nodo = nodo.siguiente;
            indice ++;
        }
        return -1;
    ...
    @Override public boolean equals(Object objeto) {
        ...
        if (!(objeto instanceof Lista)) return false;
    
        if (lista == null || lista.getLongitud() != longitud) return false;
    
        if (lista.getLongitud() == 0 && longitud == 0) return true;
    
        Nodo n = cabeza;
        int i = 0;
    
        while (n != null) {
            if (!n.elemento.equals(lista.get(i))) return false;
            n = n.siguiente;
            i++;
        }
    ...
    private Lista<T> mergeSort(Lista<T> l, Comparator<T> comparador){
              if(l.esVacia() || l.getLongitud() == 1) 
                return l;
              int enMedio = l.getLongitud() / 2 ;
              Lista<T> listota = new Lista<T>();
              Lista<T> listita;
              while(l.getLongitud() != enMedio){
                listota.agregaFinal(l.getPrimero());
                if(l.getLongitud() != 0)
                  l.eliminaPrimero();
              }
              listita = l.copia();
              return mezcla(mergeSort(listota, comparador), mergeSort(listita, comparador), comparador);
             }
    ...
    private Lista<T> mezcla(Lista<T> a, Lista<T> b, Comparator<T> comparador){
        ...
        while(a.cabeza != null && b.cabeza != null){
            int comparar = comparador.compare(a.cabeza.elemento, b.cabeza.elemento);
            if(comparar <= 0){
